...
We check both the assumptions of the beta regression model that the response variable follows a beta distribution and that the response variable is linearly related to the predictors in the appendix. 
...
betaMod <- betareg(per_dem ~ trader_joes + cabelas + cinemark + nordstrom + white_pop + 
...
From the plots below, we see that neither the votes for Democrats in a county nor the votes for Republicans in a county follows a negative binomial distribution.

```{r, include=T}
distplot(main_data$votes_dem, type="nbinom", main = "Negative binomialness plot shows the negative binomial distribution is not appropriate", xlab = "Votes for Democrats Count", conf_int = F, legend=F)

distplot(main_data$votes_gop, type="nbinom", main = "Negative binomialness plot shows the negative binomial distribution is not appropriate", xlab = "Votes for Republicans Count", conf_int = F, legend=F)
```

Below, we see that both the percentage of votes that Democrats received in a county and the percentage of votes that Republicans received in a county follow a beta distribution.

```{r, include=T}
ggplot(data=main_data, mapping=aes(x=per_dem)) +
  geom_histogram()

ggplot(data=main_data, mapping=aes(x= per_gop)) +
  geom_histogram()
```
